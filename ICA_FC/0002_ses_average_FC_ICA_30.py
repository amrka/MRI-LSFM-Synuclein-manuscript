if __name__ == '__main__':
    ...
    def help_message():
        ...
        >>> python 0002_ses_average_FC_ICA_30.py  <that contain the bids folder> \n
        Examples:
        >>> python 0002_ses_average_FC_ICA_30.py /Users/aeed/Documents/Work/M83_clearing/Manuscript_analysis
        >>> python 0002_ses_average_FC_ICA_30.py/scratch/aeed/M83_clearing
        """)
    ...
    output_dir = '{0}/FC_ICA/FC_average_ICA_30_outputdir'.format(origin_dir)
    working_dir = '{0}/FC_ICA/FC_average_ICA_30_workingdir'.format(origin_dir)

    FC_average_ICA_30_workflow = Workflow(name='FC_average_ICA_30_workflow')
    FC_average_ICA_30_workflow.base_dir = opj(experiment_dir, working_dir)
    ...
    # In[4]:
    connectivity_dir = {
        'FC_matrix_r1': 'FC_ICA/FC_ICA_metaflow_outputdir/functional_{conn_id}_ICA_30/run_1_session_{session_id}_{subject_id}/{conn_id}_matrix_ICA_*_fisherz.csv',
        'FC_matrix_r2': 'FC_ICA/FC_ICA_metaflow_outputdir/functional_{conn_id}_ICA_30/run_2_session_{session_id}_{subject_id}/{conn_id}_matrix_ICA_*_fisherz.csv',
        'FC_matrix_r3': 'FC_ICA/FC_ICA_metaflow_outputdir/functional_{conn_id}_ICA_30/run_3_session_{session_id}_{subject_id}/{conn_id}_matrix_ICA_*_fisherz.csv',
        'FC_matrix_r4': 'FC_ICA/FC_ICA_metaflow_outputdir/functional_{conn_id}_ICA_30/run_4_session_{session_id}_{subject_id}/{conn_id}_matrix_ICA_*_fisherz.csv',
    }
    ...
    FC_average_ICA_30_workflow.connect([
    ...
    FC_average_ICA_30_workflow.write_graph(graph2use='colored', format='png', simple_form=True)

    # for the cluster
    if os_name == 'CentOS Linux':
        FC_average_ICA_30_workflow.run(plugin='SLURM', plugin_args={
            'dont_resubmit_completed_jobs': True, 'max_jobs': 50, 'sbatch_args': '--mem=40G'})

    # for the laptop
    elif os_name == 'Ubuntu':
        FC_average_ICA_30_workflow.run('MultiProc', plugin_args={'n_procs': 8})

    # for the iMac
    elif os_name == 'Darwin':
        FC_average_ICA_30_workflow.run('MultiProc', plugin_args={'n_procs': 16})
